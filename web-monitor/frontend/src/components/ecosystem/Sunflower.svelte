...
>
    ...
    <div class="relative mx-auto" style="width: 24px; height: 120px;">
        ...
        <div
            class="absolute inset-x-0 top-0 bottom-0 bg-[#5D4037] border-x-2 border-[#3E2723]"
        >
            <!-- Veines de la tige -->
    ...
    >
        <!-- Veines feuille -->
        <div
            class="absolute top-1/2 left-1/2 w-full h-0.5 bg-[#1B5E20] -translate-y-1/2 rotate-12"
        ></div>
    </div>
    ...
    >
        <!-- Veines feuille -->
        <div
            class="absolute top-1/2 left-0 w-full h-0.5 bg-[#1B5E20] -translate-y-1/2 -rotate-12"
        ></div>
    </div>
    ...
    <div class="absolute -top-4 left-1/2 -translate-x-1/2">
        ...
        <div class="relative w-20 h-20">
            ...
            <!-- Centre de la fleur -->
            <div
                class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 bg-[#3E2723] border-4 border-[#5D4037] z-20 shadow-inner"
            >
                <!-- Texture centre (graines) -->
                <div class="absolute inset-0 grid grid-cols-3 gap-0.5 p-0.5">
                    {#each Array(9) as _, i}
                        <div
                            class="w-full h-full bg-[#5D4037] rounded-full"
                        ></div>
                    {/each}
                </div>
                <!-- Reflet -->
                <div
                    class="absolute top-1 left-1 w-2 h-2 bg-[#6D4C41] rounded-full"
                ></div>
            </div>
        </div>
    </div>
    ...
    <!-- Ombre portée au sol -->
    <div
        class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-16 h-4 bg-black/20 rounded-full blur-sm"
    ></div>
</div>
